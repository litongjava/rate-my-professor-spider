package com.litongjava.spider.rmp.model.base;

import com.litongjava.db.activerecord.Model;
import com.litongjava.model.db.IBean;

/**
 * Generated by java-db, do not modify this file.
 */
@SuppressWarnings({"serial", "unchecked"})
public abstract class BaseRumiSchoolDict<M extends BaseRumiSchoolDict<M>> extends Model<M> implements IBean {

	public M setId(java.lang.Long id) {
		set("id", id);
		return (M)this;
	}
	
	public java.lang.Long getId() {
		return getLong("id");
	}
	
	public M setRmpSchoolId(java.lang.Integer rmpSchoolId) {
		set("rmp_school_id", rmpSchoolId);
		return (M)this;
	}
	
	public java.lang.Integer getRmpSchoolId() {
		return getInt("rmp_school_id");
	}
	
	public M setFullName(java.lang.String fullName) {
		set("full_name", fullName);
		return (M)this;
	}
	
	public java.lang.String getFullName() {
		return getStr("full_name");
	}
	
	public M setAbbrName(java.lang.String abbrName) {
		set("abbr_name", abbrName);
		return (M)this;
	}
	
	public java.lang.String getAbbrName() {
		return getStr("abbr_name");
	}
	
	public M setBotName(java.lang.String botName) {
		set("bot_name", botName);
		return (M)this;
	}
	
	public java.lang.String getBotName() {
		return getStr("bot_name");
	}
	
	public M setRemark(java.lang.String remark) {
		set("remark", remark);
		return (M)this;
	}
	
	public java.lang.String getRemark() {
		return getStr("remark");
	}
	
	public M setCreator(java.lang.String creator) {
		set("creator", creator);
		return (M)this;
	}
	
	public java.lang.String getCreator() {
		return getStr("creator");
	}
	
	public M setCreateTime(java.util.Date createTime) {
		set("create_time", createTime);
		return (M)this;
	}
	
	public java.util.Date getCreateTime() {
		return getDate("create_time");
	}
	
	public M setUpdater(java.lang.String updater) {
		set("updater", updater);
		return (M)this;
	}
	
	public java.lang.String getUpdater() {
		return getStr("updater");
	}
	
	public M setUpdateTime(java.util.Date updateTime) {
		set("update_time", updateTime);
		return (M)this;
	}
	
	public java.util.Date getUpdateTime() {
		return getDate("update_time");
	}
	
	public M setDeleted(java.lang.Integer deleted) {
		set("deleted", deleted);
		return (M)this;
	}
	
	public java.lang.Integer getDeleted() {
		return getInt("deleted");
	}
	
	public M setTenantId(java.lang.Long tenantId) {
		set("tenant_id", tenantId);
		return (M)this;
	}
	
	public java.lang.Long getTenantId() {
		return getLong("tenant_id");
	}
	
}

